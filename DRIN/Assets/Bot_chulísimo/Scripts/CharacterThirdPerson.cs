	...
	public class CharacterThirdPerson : CharacterBase {
		...
		public override void Move(Vector3 deltaPosition, Quaternion deltaRotation) {
			...
			if (onGround) {
				...
				// Air move
				Vector3 airMove = new Vector3 (inputState.move.x * airSpeed, rigidbody.velocity.y, inputState.move.z * airSpeed);
		...
		private void Look() {
			...
			Vector3 lookDirection = isMoving? inputState.move: inputState.lookPos - rigidbody.position;
		...
		private bool Jump() {
			...
			rigidbody.velocity = new Vector3(jumpVelocity.x, jumpPower, jumpVelocity.z);
		...
		private void GroundCheck () {
			...
			Debug.DrawLine(rigidbody.position, hit.point, Color.red);
			
			// if not jumping...
			if (Time.time > jumpEndTime && rigidbody.velocity.y < jumpPower * 0.5f) {
				bool g = onGround;
				onGround = false;
